...
export default function CoderPanel() {
  ...
  async function generateAndSave() {
    if (!ticket) {
      const msg = "No ticket loaded. Use Ticketizer → Send to Coder or enter one manually.";
      setError(msg);
      toast({ variant: "destructive", title: "Missing ticket", description: msg });
      return;
    }
    setError(null);
    setSaveResult(null);
    ...
    try {
      ...
      toast({
        variant: data.ok ? "success" : "default",
        title: "Code generated & saved",
        description: `${data.generated} generated • ${data.written} written • ${data.skipped} skipped`,
      });
    } catch (e: any) {
  ...
  /** Send to Reviewer (instant handoff + durable fallback) */
  function sendToReviewer() {
    if (!ticket) {
      const msg = "No ticket loaded.";
      setError(msg);
      toast({ variant: "destructive", title: "Nothing to send", description: msg });
      return;
    }
    if (!generatedFiles?.length) {
      const msg = "Nothing to send: generate first.";
      setError(msg);
      toast({ variant: "destructive", title: "Nothing to send", description: msg });
      return;
    }
    ...
    // 1) durable path (so a page reload still works)
    localStorage.setItem("reviewer:prefill", JSON.stringify(payload));

    // 2) immediate in-page handoff (no reload required)
    window.dispatchEvent(new CustomEvent("reya:handoff", {
      detail: { target: "reviewer", payload },
    }));

    // 3) focus Roles page + Reviewer card
    navTo("roles");
    window.dispatchEvent(new CustomEvent("reya:roles-focus", {
      detail: { panel: "reviewer" },
    }));

    toast({
      variant: "success",
      title: "Sent to Reviewer",
      description: `${generatedFiles.length} file(s) forwarded.`,
    });
  }
  ...
  function adoptManualAsTicket() {
    ...
    toast({ title: "Ticket loaded", description: `${t.id} – ${t.title}` });
  }
