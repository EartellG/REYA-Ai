...
interface Ticket {
  id: string; title: string; type: "Backend" | "Frontend" | "QA";
  estimate: number; acceptance_criteria: string[]; tags: string[]
}
...
export default function TicketizerPanel() {
  ...
  useEffect(() => {
    ...
    if (seedRaw) {
      try {
        const seed = JSON.parse(seedRaw); // { idea?: string, notes?: string }
        setForm((f) => ({
          ...f,
          title: seed?.idea || f.title,
          background: seed?.notes || f.background,
        }));
      } catch { /* ignore bad JSON */ }
      localStorage.removeItem("ticketizer:seed");
    }
  }, []);
  ...
  const toMarkdown = (r: TicketizeResponse) => {
    ...
    r.user_stories.forEach((s) =>
      lines.push(`- As a **${s.role}**, I need **${s.need}**, so that **${s.why}**.`)
    );
    lines.push(`\n## Tickets`);
    r.tickets.forEach((t) => {
      ...
      <div className="grid gap-2">
        {(["title","goal","background","constraints","non_goals"] as const).map((k) => (
          <label key={k} className="grid gap-1">
            <span className="text-sm font-medium capitalize">{k.replace("_", " ")}</span>
            <textarea
        ...
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2">
            ...
            <select
              value={form.estimate_units}
              onChange={(e) =>
                setForm({ ...form, estimate_units: e.target.value as SpecInput["estimate_units"] })
              }
        ...
        <div className="space-y-3">
          ...
          <ul className="list-disc pl-6">
            {result.user_stories.map((s, i) => (
              <li key={i}>
                As a <b>{s.role}</b>, I need <b>{s.need}</b>, so that <b>{s.why}</b>.
              </li>
            ))}
          </ul>
            ...
            {result.tickets.map((t) => (
              <div key={t.id} className="border rounded p-3">
                ...
                <div className="text-xs opacity-70">
                  Estimate: {t.estimate} {form.estimate_units}
                </div>
          ...
          <div className="flex gap-2">
            <button onClick={copyMarkdown} className="px-3 py-2 rounded border">
              Copy Markdown
            </button>
          </div>
        </div>
